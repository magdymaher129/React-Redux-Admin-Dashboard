...
function BrandTable() {
  ...
  const items = useSelector((state) => state.brand.data);
  const meta = useSelector((state) => state.brand.meta);
  //console.log("metaBrand", meta.pageSize, meta.pageCount, meta.total);
  ...
  const handleIncrement = () => {
    if (currentPage < meta.pageCount) {
      ...
      dispatch(setBrandIncrement());
    }
  };
  const handleDecrement = () => {
    if (currentPage > 1) {
      setCurrentPage(currentPage - 1);
      dispatch(setBrandDecrement());
    } else {
      setCurrentPage(1);
  ...
  const handlePageSize = (e) => {
    ...
    setCurrentPage(1);
    dispatch(setBrandPage(1));
    dispatch(setBrandSize(parseInt(e.target.value)));
  };
  ...
  return (
    <div className='mt-4'>
      ...
      <h3 className='text-center text-capitalize text-default my-4'> Brand </h3>
              ...
              <thead className='table-light '>
                <tr>
                  ...
                  <th className='text-center ' style={{ maxWidth: "200px" }}>
                    <Link to='/addbrand' className='btn btn-primary btn-sm'>
                      Add Brand
                    </Link>
                  </th>
                </tr>
